class Matriz:
  ...
  def __init__(self, filas: int, columnas: int):
    if filas <= 0 or columnas <= 0:
      ...
      raise Exception("El numero de filas y columnas tienen que ser mayores a 0")
  ...
  def _boundcheck_(self, fila: int, columna: int):
    if fila > self.filas or columna > self.columnas or fila <= 0 or columna <= 0:
      raise Exception(f"Sobrepaso de indice, se pidió la posición ({fila},{columna}) en una matriz de {self.filas}x{self.columnas}")
